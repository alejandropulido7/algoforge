...
#property description "Port of Python ta library MACD indicator"
                ...
                const long &tick_volume[], const long &volume[], const int &spread[])
{
   ...
   for(int i=start; i<rates_total && !IsStopped(); i++)
   {
      ...
      double macd=ef-es;
      ExtBuffer0[i]=macd;
      if(i==InpSlow-1) sig=macd;
      else sig += (2.0/(InpSignal+1))*(macd-sig);
      if(i>=InpSlow-1+InpSignal-1){ ExtBuffer1[i]=sig; ExtBuffer2[i]=macd-sig; }
   }
   return(rates_total);
}
