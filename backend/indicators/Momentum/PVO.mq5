...
#property description "Port of Python ta library PVO indicator"
...
int OnInit()
{
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "PVO");
   PlotIndexSetInteger(0, PLOT_DRAW_TYPE, DRAW_LINE);
   PlotIndexSetString(0, PLOT_LABEL, "PVO");
                ...
                const long &tick_volume[], const long &volume[], const int &spread[])
{
   ...
   {
      if(i==0){ ef=tick_volume[0]; es=tick_volume[0]; }
      else { ef += (2.0/(InpFast+1))*(tick_volume[i]-ef); es += (2.0/(InpSlow+1))*(tick_volume[i]-es); }
