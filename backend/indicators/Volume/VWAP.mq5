...
#property description "Port of Python ta library VWAP indicator"
                ...
                const long &tick_volume[], const long &volume[], const int &spread[])
{
   ...
   for(int i=start; i<rates_total && !IsStopped(); i++)
   {
      ...
      {
         double tp=(high[i-k]+low[i-k]+close[i-k])/3.0;
         sp += tp*tick_volume[i-k];
         sv += tick_volume[i-k];
      }
      ExtBuffer0[i]= sv!=0.0 ? sp/sv : EMPTY_VALUE;
   }
   return(rates_total);
}
